...
test("a pre-pagination size failure resumes the existing completed job",async t=>{
  ...
  assert.ok(requests[0].url.endsWith("/v1/pipeline/jobs/job-existing/import"));
});
...
test("oversized chunked pages cancel their reader without accepting partial JSON",async t=>{
  ...
  await assert.rejects(()=>breakfast({BREAKFAST_KEY:"test-secret"},"/v1/pipeline/jobs/job-existing/import"),e=>e.status===422);
